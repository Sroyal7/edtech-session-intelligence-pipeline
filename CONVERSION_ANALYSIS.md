# Conversion Analysis: Intent Scoring & 1-Line Explanations
...
**Primary Signal:** Willing to switch roles for work-life balance (family comes first) + income target already stretches to 18-20 LPA.
...
**Primary Signal:** "I'm tired of applying and hoping" (emotional pain) + structured roadmap addresses her exact gap (skills + guidance).
...
**Primary Signal:** Self-respect through independence is the driver (identity), not financial desperation; needs emotional validation, not just income strategy.
...
**Primary Signal:** "Every month I'm more stressed" = urgency spike; commitment to structured path is near certain if timeline is compressed.
...
**Primary Signal:** "My husband supports education" (enabler) but he also decides = gating factor; sell the outcome to both, not just the individual.
...
**Primary Signal:** Constraint (office mandate) + opportunity (remote upskilling) align perfectly; structured support = removes only remaining friction.
...
**Primary Signal:** Needs reassurance that AI skills are learnable + relevant, not just skill training; positioning is the key here.
...
**Primary Signal:** "I want to support my kids' education myself" = identity + financial anchor combined; emotional driver is exceptionally strong.
...
**Primary Signal:** Education investment conflict; she needs to know how to balance MBA + upskilling, not just which to pick.
...
**Primary Signal:** "My family is counting on me" (external accountability) + "I can commit 2 hours daily" (execution readiness) = conversion is nearly guaranteed.
...
- Family gatekeeping (need buy-in, not just individual conviction)
- Competing priorities (education, exploration, other goals)
- Knowledge gaps (AI anxiety, skill confidence)
- "Exploring" vs. "desperate" mindset

### Key Success Factors for Conversion
1. **Emotional pain + financial pain alignment** = highest urgency
2. **Family support/gating** = requires dual-persona messaging
3. **Identity + income combo** = strongest driver (self-respect + security)
4. **Timeline clarity** = shortens indecision phase
5. **Constraint + opportunity match** = removes friction

---

## How AI Generates These Analyses

The EdTech system processes raw consultation notes and outputs:

1. **Signal Extraction** – Identifies all 10+ data points (location, exp, income, timeline, constraints, pain points, motivations)
2. **Intent Scoring** – Weighs emotional pain + financial pressure + timeline + commitment readiness → HIGH/MEDIUM/LOW
3. **1-Line Explanation** – Synthesizes primary driver(s) and blocker(s) into a single actionable insight
4. **Primary Signal** – Highlights the single most influential factor for that specific profile
5. **Talk Track** – Generates follow-up conversation hooks based on the conversion readiness
